// Entire file content will be replaced, so the SEARCH block is minimal.
// Using a very common and stable line.
namespace ComicRentalSystem_14Days
{
    public partial class MainForm : BaseForm
    {
        ...
        {
            if (dgvAvailableComics == null || _comicService == null) return;
        ...
        private void SetupUIAccessControls()
        {
            ...
            if (mainContentPanel == null) { _logger?.LogError("設定UI控制項存取權限失敗：mainContentPanel 為空。"); return; }

            if (leftNavPanel != null) leftNavPanel.Visible = isAdmin;
            if (memberViewTabControl != null) memberViewTabControl.Visible = !isAdmin;

            // Admin specific panel visibility is handled by SelectNavButton
            if (_adminDashboardControl != null && !isAdmin) _adminDashboardControl.Visible = false;

            // Visibility of these controls for Admin is handled by SelectNavButton (ShowAdminComicManagementView)
            if (lblAvailableComics != null && !isAdmin) lblAvailableComics.Visible = true; // visible in member view
            if (cmbAdminComicFilterStatus != null) cmbAdminComicFilterStatus.Visible = isAdmin; // Only for admin
            if (dgvAvailableComics != null) dgvAvailableComics.Visible = true; // Visible for both, content changes
            ...
            if (isAdmin)
            {
                ...
                if (btnNavDashboard != null) SelectNavButton(btnNavDashboard);
                else if (btnNavComicMgmt != null) SelectNavButton(btnNavComicMgmt);
            }
            else // Member
            {
                this.Text = Constants.UI.FormTitles.MemberBaseTitle;
                if (lblAvailableComics != null) lblAvailableComics.Text = "請選擇下方漫畫進行租借：";
                if (lblMyRentedComicsHeader != null) lblMyRentedComicsHeader.Text = "您目前租借的項目：";
        ...
        {
            if (dgvMyRentedComics == null || _currentUser == null || _comicService == null || _memberService == null || _logger == null)
            ...
            try
            {
                ...
                if (allComics == null) { _logger?.LogWarning("載入我的租借漫畫失敗：_comicService.GetAllComics() 回傳為空。"); ClearDgvMyRentedComics(); return; }
        ...
        private void 會員管理ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (Program.AppAuthService != null && this._comicService != null && _memberService != null)
            {
        ...
        private async void btnRentComic_Click(object? sender, EventArgs e)
        {
            ...
            if (selectedComic == null) { MessageBox.Show("選擇的項目無效或不是有效的漫畫資料。", Constants.UI.MessageBoxTitles.Error, MessageBoxButtons.OK, MessageBoxIcon.Error); _logger?.LogError("租借漫畫按鈕點擊事件：選取的項目為空或不是有效的漫畫物件。"); return; }
            if (selectedComic.IsRented) { _logger?.Log($"租借漫畫按鈕點擊事件：使用者 '{_currentUser.Username}' 嘗試租借漫畫 '{selectedComic.Title}' (ID: {selectedComic.Id})，但該漫畫已被租借。"); MessageBox.Show($"漫畫 '{selectedComic.Title}' 已經被借出。", Constants.UI.MessageBoxTitles.Information, MessageBoxButtons.OK, MessageBoxIcon.Information); LoadAvailableComics(); return; }
            ...
            using (RentalPeriodForm rentalDialog = new RentalPeriodForm(today.AddDays(3), today.AddMonths(1)))
            {
                if (rentalDialog.ShowDialog(this) == DialogResult.OK)
                {
                    ...
                    if (member == null) { _logger?.LogWarning($"租借漫畫按鈕點擊事件：找不到使用者名稱為 '{_currentUser.Username}' 的會員。"); MessageBox.Show($"找不到使用者 '{_currentUser.Username}' 對應的會員資料。", Constants.UI.MessageBoxTitles.Error, MessageBoxButtons.OK, MessageBoxIcon.Error); return; }
        ...
        {
            if (_currentUser == null || _currentUser.Role != UserRole.Admin || this.dgvAvailableComics == null || e.ColumnIndex < 0 || e.ColumnIndex >= this.dgvAvailableComics.Columns.Count) return;
        ...
        private void cmbAdminComicFilterStatus_SelectedIndexChanged(object? sender, EventArgs e) { if (_currentUser == null || _currentUser.Role != UserRole.Admin) return; ApplyAdminComicsView(); }
        ...
        private void ShowAdminComicManagementView()
        {
            ...
            if (lblAvailableComics != null && lblAvailableComics.Parent != mainContentPanel) mainContentPanel.Controls.Add(lblAvailableComics);
            if (cmbAdminComicFilterStatus != null && cmbAdminComicFilterStatus.Parent != mainContentPanel) mainContentPanel.Controls.Add(cmbAdminComicFilterStatus);
            if (dgvAvailableComics != null && dgvAvailableComics.Parent != mainContentPanel) mainContentPanel.Controls.Add(dgvAvailableComics);

            var panelWidth = mainContentPanel.ClientSize.Width; var panelHeight = mainContentPanel.ClientSize.Height; const int margin = 8;
            if (lblAvailableComics != null) { lblAvailableComics.SetBounds(margin, margin, panelWidth - 2 * margin, 28); lblAvailableComics.Visible = true; }
            if (cmbAdminComicFilterStatus != null) { int comboWidth = 120, comboHeight = 23, comboX = panelWidth - comboWidth - margin, comboY = (lblAvailableComics?.Bottom ?? margin) + 8; cmbAdminComicFilterStatus.SetBounds(comboX, comboY, comboWidth, comboHeight); cmbAdminComicFilterStatus.Visible = true; }
            if (dgvAvailableComics != null) { int dgvY = (cmbAdminComicFilterStatus?.Bottom ?? lblAvailableComics?.Bottom ?? margin) + 8, dgvHeight = panelHeight - dgvY - margin; dgvAvailableComics.SetBounds(margin, dgvY, panelWidth - 2 * margin, dgvHeight); dgvAvailableComics.Visible = true; dgvAvailableComics.BringToFront(); }
        ...
        {
            if (_currentSelectedNavButton != null) { _currentSelectedNavButton.BackColor = ModernBaseForm.SecondaryColor; _currentSelectedNavButton.ForeColor = ModernBaseForm.TextColor; _currentSelectedNavButton.Font = ModernBaseForm.ButtonFont ?? new System.Drawing.Font("Segoe UI Semibold", 9.75F); }
            ...
            if (dgvAvailableComics != null && _currentUser?.Role == UserRole.Admin && selectedButton != btnNavComicMgmt) { /* dgvAvailableComics.Visible = false; */ } // Special handling for admin dgv visibility
        ...
        {
            if (_currentUser == null || _currentUser.Role != UserRole.Admin || dgvAvailableComics == null || e.RowIndex < 0 || e.RowIndex >= dgvAvailableComics.Rows.Count) return;
            ...
            {
                if (comicStatus.Status == Constants.ComicStatuses.Rented && comicStatus.ReturnDate.HasValue)
                {
        ...
        {
            if (dgvMyRentedComics == null || e.RowIndex < 0 || e.RowIndex >= dgvMyRentedComics.Rows.Count) return;
            DataGridViewRow row = dgvMyRentedComics.Rows[e.RowIndex];
            if (row.DataBoundItem == null) return;
            DateTime? returnDate = null;
            try { var val = row.DataBoundItem.GetType().GetProperty(Constants.DataPropertyNames.ExpectedReturnDate)?.GetValue(row.DataBoundItem, null); if (val != null && val != DBNull.Value) returnDate = Convert.ToDateTime(val); }
        ...
        {
            if (_comicService == null || dgvAvailableComics == null || _currentUser == null) { _logger?.LogWarning("套用可租借漫畫篩選器：關鍵元件為空，略過篩選。"); return; }
            try
            {
                _logger?.Log("正在套用可租借漫畫篩選器…");
                string placeholder = Constants.UI.Placeholders.SearchComicsPlaceholder;
                string searchText = (txtSearchAvailableComics != null && txtSearchAvailableComics.Text != placeholder && !string.IsNullOrWhiteSpace(txtSearchAvailableComics.Text)) ? txtSearchAvailableComics.Text.Trim().ToLowerInvariant() : "";
    ...
    public partial class MainForm : BaseForm
    {
        ...
        private async void MainForm_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                SetupUIAccessControls(); // Determines visibility based on role, affects which controls are relevant
                InitializeMainDataGridView(); // Sets up dgvAvailableComics columns based on role

                // SubscribeToControlEvents should ideally happen after controls that might trigger them are initialized
                // and before data loading that might trigger those events without user interaction yet.
                SubscribeToControlEvents();

                InitializeFilterControlsAndStyles(); // Includes search box placeholders and other filter setups

                await LoadInitialDataAsync(); // Loads data based on role

                this._logger?.Log("主表單載入完成。");
            }
            catch (Exception ex)
            {
                _logger?.LogError("MainForm_Load 期間發生未預期的錯誤。", ex);
                MessageBox.Show($"應用程式載入期間發生嚴重錯誤: {ex.Message}\n應用程式可能無法正常運作。", "載入錯誤", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void InitializeAdminDashboardControl()
        {
            if (_currentUser.Role == UserRole.Admin && _comicService != null && _memberService != null && _logger != null)
            {
                ...
                {
                    Dock = DockStyle.Fill,
                    Visible = false // Will be made visible by SelectNavButton or SetupUIAccessControls
                };
                if (mainContentPanel != null) mainContentPanel.Controls.Add(_adminDashboardControl);
            }
        }
        ...
        {
            this._logger?.Log("正在初始化主 DataGridView...");
            SetupDataGridView(); // This existing method sets up dgvAvailableComics based on role
        }
        ...
        private async Task LoadInitialDataAsync()
        {
            ...
            {
                 _logger?.Log("[TARGETED_RUNTIME] Applying visibility settings for availableComicsTabPage and dgvAvailableComics in LoadInitialDataAsync for Member.");
                // Visibility for these should be handled by SetupUIAccessControls now
                // if (this.availableComicsTabPage != null) this.availableComicsTabPage.Visible = true;
                // if (this.dgvAvailableComics != null) { this.dgvAvailableComics.Visible = true; this.dgvAvailableComics.BringToFront(); }

                if (_comicService != null) await _comicService.ReloadAsync();
                _logger.Log($"LoadInitialDataAsync (Member) [After ReloadAsync]: _comicService reports {_comicService?.GetAllComics().Count ?? 0} comics.");
                LoadAvailableComics(); // Populates dgvAvailableComics for member
                LoadMyRentedComics();  // Populates dgvMyRentedComics for member
            }
            // Initial selection for dgvAvailableComics if it has items
            if (this.dgvAvailableComics != null && this.dgvAvailableComics.Rows.Count > 0)
                 dgvAvailableComics_SelectionChanged(this, EventArgs.Empty);
        }
        ...
        private void InitializeFilterControlsAndStyles()
        {
            ...
            if (this.cmbAdminComicFilterStatus != null && _currentUser.Role == UserRole.Admin)
            {
                ...
                this.cmbAdminComicFilterStatus.Items.Add("已租借"); // Assuming "Rented" was a typo in original code for actual value
                this.cmbAdminComicFilterStatus.Items.Add("可租借"); // Assuming "Available" was a typo
            ...
            {
                if (btnRentComic != null) StyleModernButton(btnRentComic);
                // dgvAvailableComics and dgvMyRentedComics styling is complex and might be better handled
                // within their specific setup or after data load if styles depend on content.
                // For now, direct styling calls like StyleModernDataGridView are kept here if they were global.
                if (dgvAvailableComics != null) StyleModernDataGridView(dgvAvailableComics); // General style
                if (dgvMyRentedComics != null) StyleModernDataGridView(dgvMyRentedComics); // General style
            }
        ...
        private async void ComicService_ComicsChanged(object? sender, EventArgs e)
        {
            ...
            {
                _logger?.LogError("處理 ComicsChanged 事件時發生未預期的錯誤。", ex);
                // Avoid showing MessageBox here if it's a background refresh,
                // unless it's critical for the user to know immediately.
                // Logging is the primary way to capture this.
            }
        }
        ...
        private async void btnRentComic_Click(object? sender, EventArgs e) // Method signature is already async void
        {
            if (_currentUser == null || _comicService == null || _memberService == null || _logger == null)
            {
            ...
            using (RentalPeriodForm rentalDialog = new RentalPeriodForm(minRentalReturnDate, maxRentalReturnDate))
            {
                if (rentalDialog.ShowDialog(this) == DialogResult.OK)
                {
                    ...
                    try
                    {
                        await _comicService.UpdateComicAsync(selectedComic); // THE ONLY CHANGE IS HERE
                        _logger?.Log($"漫畫 '{selectedComic.Title}' (ID: {selectedComic.Id}) 已租借給會員 ID {member.Id} (使用者名稱: {_currentUser.Username}) 至 {selectedReturnDate:yyyy-MM-dd}。");
                    ...
                    catch (Exception ex)
                    {
                        ...
                        selectedComic.IsRented = false; // Attempt to revert state
                        selectedComic.RentedToMemberId = 0; // Attempt to revert state
        ...
        {
            if (_adminDashboardControl != null) _adminDashboardControl.Visible = false;
            if (memberViewTabControl != null) memberViewTabControl.Visible = false; // For member view

            // Admin Comic Management View specific controls (if directly on mainContentPanel)
            if (lblAvailableComics != null) lblAvailableComics.Visible = false;
            if (cmbAdminComicFilterStatus != null) cmbAdminComicFilterStatus.Visible = false;
            if (dgvAvailableComics != null && _currentUser?.Role == UserRole.Admin) // Check role before hiding dgv used by member too
            {
                 // If dgvAvailableComics is shared, its visibility is tricky.
                 // For now, assume it's hidden if it was part of a specific admin view.
                 // This might need adjustment based on whether dgvAvailableComics is reused for member view.
                 // Based on SetupUIAccessControls, dgvAvailableComics is made visible for member view too.
                 // So, only hide it if it's specifically being used for the *admin comic management view*.
                 // This method is called before switching to a *new* admin view.
            }
            // If dgvAvailableComics is inside a panel that's hidden, this direct hiding might be redundant.
        }
        ...
        {
            HideAllMainContentViews();
            // Ensure these controls are parented to mainContentPanel if they aren't already by design
            if (lblAvailableComics != null && mainContentPanel != null && lblAvailableComics.Parent != mainContentPanel)
            {
                mainContentPanel.Controls.Add(lblAvailableComics);
            }
            if (cmbAdminComicFilterStatus != null && mainContentPanel != null && cmbAdminComicFilterStatus.Parent != mainContentPanel)
            {
                mainContentPanel.Controls.Add(cmbAdminComicFilterStatus);
            }
            if (dgvAvailableComics != null && mainContentPanel != null && dgvAvailableComics.Parent != mainContentPanel)
            {
            ...
            if (mainContentPanel != null)
            {
                ...
                if (lblAvailableComics != null)
                {
                    lblAvailableComics.SetBounds(margin, margin, panelWidth - 2 * margin, 28);
                    lblAvailableComics.Visible = true;
                }
                ...
                if (cmbAdminComicFilterStatus != null)
                {
                    ...
                    cmbAdminComicFilterStatus.SetBounds(comboX, comboY, comboWidth, comboHeight);
                ...
                if (dgvAvailableComics != null)
                {
                    ...
                    dgvAvailableComics.SetBounds(margin, dgvY, panelWidth - 2 * margin, dgvHeight);
        ...
        private void SelectNavButton(Button selectedButton)
        {
            ...
            // Default to hiding specific admin comic view controls unless that view is selected.
            // More robust view management would involve panels for each view.
            if (lblAvailableComics != null && selectedButton != btnNavComicMgmt) lblAvailableComics.Visible = false;
            if (cmbAdminComicFilterStatus != null && selectedButton != btnNavComicMgmt) cmbAdminComicFilterStatus.Visible = false;
            // dgvAvailableComics is more complex as it's used by member view too.
            // Its visibility is managed by SetupUIAccessControls and the tab control for members.
            // For admin, it's part of ShowAdminComicManagementView or hidden.
            if (_currentUser?.Role == UserRole.Admin && dgvAvailableComics != null && selectedButton != btnNavComicMgmt)
            {
                 // Only hide if it's the admin view and not the comic mgmt view.
                 // If it's also used by dashboard, this logic needs care.
                 // For now, assume dashboard has its own controls (_adminDashboardControl).
                 if(_adminDashboardControl == null || !_adminDashboardControl.Visible) {
                    //dgvAvailableComics.Visible = false; // This might be too aggressive if dashboard also uses it.
                 }
            }
            ...
            {
                ShowAdminComicManagementView();
            }
            else if (selectedButton == btnNavMemberMgmt)
            {
                HideAllMainContentViews(); // Hide dashboard/comic management view
